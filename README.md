# Nest.js + React + AWS 学习项目

从 0 构建一个 **Nest.js + React + AWS (Lambda & EC2) + Aurora Serverless v2 + Prisma** 的完整全栈学习项目。

支持 **Lambda 和 EC2 双部署模式**，前端可动态切换后端服务。

## 🌟 项目亮点

- **双部署架构**：同一后端代码同时部署到 Lambda（Serverless）和 EC2（传统），对比两种模式的差异
- **动态服务切换**：前端一键切换后端服务，实时体验不同架构的响应速度和特性
- **完整全栈实现**：从数据库到前端的完整技术栈，包含 Aurora Serverless v2 + Prisma ORM
- **成本优化实践**：学习如何通过架构设计和资源管理控制云服务成本
- **生产级配置**：包含 PM2 进程守护、Nginx 反向代理、CloudWatch 日志等生产环境实践
...
| 部署模式 1 | AWS Lambda + API Gateway (Serverless) |
| 部署模式 2 | EC2 (t4g.micro) + PM2 + Nginx (传统部署) |
| 部署工具 | AWS SAM / 手动脚本 |
| ORM | Prisma |
| 数据库 | Aurora PostgreSQL Serverless v2 |
| 数据库管理 | Supabase (只读查看) |
| 前端 | React + TypeScript + Vite |
| 前端特性 | 动态服务切换（Lambda ⇄ EC2） |
| 静态托管 | S3 + CloudFront (可选) |
...
访问 http://localhost:5173 即可体验服务切换功能。
...
详细部署文档见 [backend/docs/](backend/docs/) 目录。
...
### Phase 9.5: EC2 部署（已完成，与 Lambda 并行）
- ✅ EC2 实例配置（t4g.micro + ARM64 + Ubuntu 24.04）
- ✅ PM2 进程守护和自动重启
- ✅ Nginx 反向代理配置
- ✅ 自动化部署脚本（deploy.sh + pm2.config.js）
- ✅ Aurora Serverless v2 数据库连接
...
- ✅ Item CRUD 界面（创建、列表、删除）
- ✅ GitHub 用户信息获取
- ✅ **服务切换功能**：一键切换 Lambda 和 EC2 后端
...
| Phase 8 | **NAT Gateway** | **~$33/月** | 按小时计费 + 流量费 |
| **Phase 9.5** | **EC2 (t4g.micro)** | **~$5/月** | 与 Lambda 并行部署 |
| Phase 10-11 | S3 + CloudFront | $0.5-2/月 | 存储和流量费（可选） |
| **学习期总成本** | **1-2 周完成** | **~$20-40** | 及时清理是关键 |

**实际运行成本**（如长期保留）：
- Aurora Serverless v2: ~$43/月
- NAT Gateway: ~$33/月（可选，用于 Lambda 访问外网）
- EC2 实例: ~$5/月
- 总计: ~$81/月（含 NAT）或 ~$48/月（不含 NAT）
...
- [x] Phase 8: 创建 NAT Gateway（用于 Lambda 访问外网）
- [x] **Phase 9.5: EC2 部署（PM2 + Nginx）**
- [x] **Phase 10: 前端开发（React + 服务切换功能）**
- [ ] Phase 11: 前端部署（S3 + CloudFront）
- [ ] Phase 12: 完整联调与清理
